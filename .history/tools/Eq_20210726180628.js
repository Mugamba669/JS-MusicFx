...
var controls = {
    ...
    parallax:true,
    speed:600,
    effect:"coverflow",// flip coverflow cube fade
    gradCursor:true,
    pagination: {
...
const modes = ['Flip','Coverflow','Cube','Fade']
var gui = new dat.GUI();
const updateControls = function(modal){
    controls.effect = modal;
}
gui.add(cont,'slideSpeed',0,1000).onChange(e => controls.speed = e);
var folder = gui.addFolder('Models').add(cont,'modals').options(modes).onChange(()=>  updateControls(cont.modals));
folder.open();
...
// Default selection
$("#bars-visual").hide().css("transform", "translateY(115px)");;
$("#spec-visual").hide();
$("#up-bars-visual").hide();
$("#dw-bars-visual").hide();
$(".spiral").show();
