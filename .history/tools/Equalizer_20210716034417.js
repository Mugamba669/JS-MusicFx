...
class Equalizer {
    ...
    constructor() {
        //    this.queue = [];
            ...
            window.onload = ()=>{
                ...
                this._('.image-container').style.transform = 'scale(6)';
              
            }
            this.audio.onpause = ()=>{
                this._('#trackId').style.animationPlayState = 'paused';
                this._('.image-container').style.transform = 'scale(0.7)';
            }

            this.audio.onended = ()=>{
                this._('.image-container').style.transform = 'scale(1)';
                this._('#trackId').style.animationPlayState = 'paused';
            }

            this.audio.onplay = ()=>{
                this._('.image-container').style.transform = 'scale(2)';
    ...
    getFile(selector) {
        ...
        // this.audio.addEventListener('playing',this.trackId,false);
    ...
    getPlaylist(selector){
       ...
       $(selector).on("change",function(e){
        ...
        console.log(($('.title').text()).length)
               ...
               var icon = $("<b></b> ").addClass("fa fa-music");
               $("<p></p>").text("  "+data.name).addClass('list-tile').prepend(icon).append("<hr/>").appendTo(".list").click(function(){
                   $("#play").hide();
                    $("#pause").show();
       ...
       var audioSession = (index)=>{
           // on playing
           $(that.audio).on("playing",function(){
               $("#play").hide()
               $("#pause").show()
           });
           // on ended
