           ...
           playlist.map((data,index)=>{
            ...
            ID3.loadTags(url, () => {
               ...
               var cover = $('<img/>').attr('src', that.base64Cover(image)).addClass('coverArt');
                // var label = $('<label></label>').attr("for","list").text("  "+that.listTracks(title)).addClass('list-tile');
               $("<input/>").text("  "+that.listTracks(title)).addClass('list-tile').prepend(cover).appendTo(".list").click(function(){
           $(".plist").removeClass("w3-show").addClass('w3-hide')
           that._("#size").textContent = that.fileInfo.size(data);
                   that.audio.src = url;
                   that.audio.play();
                //    $(this).clicked == true ? $(this).addClass('active'):$(this).removeClass('active')
