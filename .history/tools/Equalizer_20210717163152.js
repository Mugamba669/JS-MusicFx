        ...
        $('.total').text(playlist.length);
           playlist.map((data,index)=>{
            ...
            ID3.loadTags(url, () => {
               ...
               $("<p></p>").text("  "+that.listTracks(title)).addClass('list-tile').prepend(cover).appendTo(".list").click(function(){
...
function renderCanvas(music) {
    ...
    */
    ...
    St_treble.frequency.value = 30000;
    St_treble.gain.value = 200;
    var valueSt = 0;
    source.connect(St_treble)
    St_treble.connect(audioCtx.destination)
    // St_treble_boost.connect(audioCtx.destination)
    // analyser.connect(audioCtx.destination)
    $('#stereo').on('input',function(){
        valueSt = $(this).val();
        $("#st-out").text(parseFloat(valueSt).toFixed(2));
        St_treble.frequency.value = valueSt;
        // St_treble_boost.gain.setValueAtTime(valueSt,audioCtx.currentTime)
    });
    /*
    Bass node 
    */
