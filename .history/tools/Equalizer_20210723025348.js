...
class Equalizer {
    ...
    //    show pop up
       ...
       $(selector).on("change",function(e){
        ...
        //  track next
        that.audio.onended =  ()=>{  nextTrack(index,arrayList);}
        // click to next track
        $('#nextTrack').show().on('click',function(){
            nextTrack(index,arrayList);
        });
        //  click to back to prev track
        $('#prevTrack').show().on('click',function(){
            prevTrack(index,arrayList);
        });
    ...
    //    next track function
    const nextTrack = function(index,arrayList){
        var next = index++;
       var url = URL.createObjectURL(arrayList[next]);
        ...
        that.trackInfo(tags,arrayList[next]);
        } , {
        dataReader: FileAPIReader(arrayList[next]),
        tags: ["title", "artist", "picture", "album","genre"]
          });
       }

    //    previous track function
      const prevTrack = function(index,arrayList){
        var prev = index--;
       var url = URL.createObjectURL(arrayList[prev]);
        ...
        that.audio.play();
        that.base64Image(image);
        that.trackInfo(tags,arrayList[prev]);
        } , {
        dataReader: FileAPIReader(arrayList[next]),
        tags: ["title", "artist", "picture", "album","genre"]
          });
