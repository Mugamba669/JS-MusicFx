...
class Equalizer {
           ...
           $(this.audio).on("ended", function() {
            ...
            this.trackListBody = (url,file)=>{
                ID3.loadTags(url,()=>{
                    ...
                    dataReader:FileAPIReader(file),
                    tags:['picture','title','artist','album']
                })
            }
            this.listTracks = (url,file)=>{
       ...
       $(selector).on("change",function(e){
        ...
        $('.total').text(index+1);
            ...
            (index < (playlist.length -1))?showLoader(index,playlist):hideLoader(time,playlist);
               ...
               var cover = $('<img/>').attr('src', that.base64Cover(url,data)).addClass('coverArt');
               $("<p></p>").text("  "+ that.listTracks(url,data)).addClass('list-tile').prepend(cover).appendTo(".list").click(function(){
        //  track next
        that.audio.onended =  ()=>{ 
            var next = index++;
    ...
    //    track name
    function trackName(url,data){
         ID3.loadTags(url, () => {
                var tags = ID3.getAllTags(url);
                var image = tags.picture;

                var title = tags.title;
                that.listTracks(title,data)
