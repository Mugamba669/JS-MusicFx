        ...
        //    this.queue = [];
           ...
           $(this.audio).on("ended", function() {
            ...
            this.fileInfo = {
                ...
                trackId: (file) => {
                    var url = this.srcURL(file);
                   this.dataSend.url = url;
                    this.audio.src = url;
                    // this.fs.writeFile('../info.json',JSON.stringify(this.dataSend.url),(error)=>{
        ...
        btns.forEach((item, index) => {
            item.onclick = ()=>{
               if( index == 0){
                ...
                //  window.cancelAnimationFrame(animationFrameResult);
               } else{
                $("#play").hide()
                $("#pause").show()
                this.audio.play();
...
function renderCanvas(audioCtx,music) {
    ...
    */
    ...
    renderDownFrame();
    spiralVisual();
    console.log(animationFrameResult)
}
...
let animationFrameResult;
/*			Rendering the bars-Canvas-visual   */
 function renderSpec() {
  animationFrameResult =  window.requestAnimationFrame(renderSpec);
...
function renderFrame() {
  animationFrameResult =  window.requestAnimationFrame(renderFrame);
...
function renderUpFrame() {
   animationFrameResult = window.requestAnimationFrame(renderUpFrame);
    var freqDomain = new Uint8Array(analyser.frequencyBinCount);
    analyser.getByteFrequencyData(freqDomain);
    //analyser.getByteTimeDomainData(freqDomain);
...
function renderDownFrame() {
   animationFrameResult = window.requestAnimationFrame(renderDownFrame);
    var freqDomain = new Uint8Array(analyser.frequencyBinCount);
    analyser.getByteFrequencyData(freqDomain);
    //analyser.getByteTimeDomainData(freqDomain);
...
var spiralVisual = function() {
  animationFrameResult = window.requestAnimationFrame(spiralVisual);
    var freqDomain = new Uint8Array(analyser.frequencyBinCount);
    analyser.getByteFrequencyData(freqDomain);
