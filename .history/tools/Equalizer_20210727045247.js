...
if (that.fileInfo.type(data) == "audio/mpeg") {
        ...
        that.audio.ontimeupdate = ()=>{
            ...
            if(result == 41){
            $(".bottom-container").show().addClass('active').delay(30000,function(){
               url = that.srcURL(arrayList[index+1]);
                $(".next-track").text(showNextTrack(url,arrayList[index+1]))
            }).removeClass('active');
            }
        }
        that.audio.onended =  ()=>{ 
    ...
    function showNextTrack(url,data){
        ID3.loadTags(url, () => {
            var tags = ID3.getAllTags(url);
              return that.listTracks(tags.title,data);
        } , {
        dataReader: FileAPIReader(data),
        tags: ["title"]
          });
