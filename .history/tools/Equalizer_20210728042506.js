    ...
    function showNextTrack(url,data){
        ID3.loadTags(url, () => {
            ...
            for (let index = 0; index < image.data.length; index++) {
                base64String += String.fromCharCode(image[index]);
            }
            const cover = $("<img/>").props("src","data:image"+image.format+";base64,"+window.btoa(base64String)).addClass('coverArt'); 
            const defaultCover = $("<img/>").attr("src","../images/pAudio.jpeg").addClass('coverArt');
            var tags = ID3.getAllTags(url);
              switch (tags.title) {
                  case undefined:
                    $(".bottom-container").show(function(){
                        $(".bottom-sheet").addClass('active',function(){
                            // display name
                                $(".next-track").text(`${that.fileInfo.name(data)}`).pr
                  ...
                  default:
                     $(".bottom-container").show(function(){
                        $(".bottom-sheet").addClass('active',function(){
                            // display name
                                $(".next-track").text(`Next-Track: ${tags.title}`)
