...
class Equalizer {
    ...
    constructor() {
            ...
            // this.fs = require('fs')
            this.tune = () => {
                // animate the lyrics container
                var h = 0;
                // setInterval(function(){
                //     h += 10;
                //     $("pre").css("transform","translateY(-"+h+"%)")
                // },3000)
                this._('.title').classList.add('active');
                this._('.artist').classList.add('active');
                this._('.album').classList.add('active');
            ...
            this.fileInfo = {
                ...
                trackId: (file) => {
                    ...
                    ID3.loadTags(url, () => {
                       ...
                       $(".lyrics-track").text(`Lyrics - ${this.listTracks(tags.title,file)}`)
                    //    image manuplation
...
if (that.fileInfo.type(data) == "audio/mpeg") {
        ...
        $(".lyrics-track").text(`Lyrics - ${that.listTracks(title,data)}`)
