                ...
                trackId: (file) => {
                    ...
                    ID3.loadTags(url, () => {
                        ...
                        $(".auto-off").click(function() {
                            var result = window.confirm('By enabling this feature the system will automatically select for you the equalizer tune depending on the track`s genre.');
                            switch (result) {
                                case true:
                                    $(".auto-off").hide();
                                    $(".auto-on").show();
...
function renderCanvas(music) {
    ...
    */
    ...
    //     source.disconnect(treble);
    //     treble.disconnect(limit);
    //     limit.disconnect(audioBoost);
    //     audioBoost.disconnect(balance);
    //     balance.disconnect(analyser);
...
function renderSpec() {
    ...
    for (var i = 0; i < canvas.width; i++) {
        ...
        speContext.fillRect(i * specX, specHeight, 2.7, 2.7);
    }
}
/*			Rendering the bars-Canvas-visual   */
